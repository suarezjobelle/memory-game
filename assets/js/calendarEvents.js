...
var renderPopup = function(jsEvent, start, end, calEvent) {
  ...
  $(".start-time").html(
    ' <p><i class="fa fa-play" aria-hidden="true"></i>' +
      (start.isSameOrBefore(moment()) ? "Started" : "Starts") +
      "</p>" +
      '<time datetime="' +
      start.format() +
      '">' +
  ...
  if (calEvent) {
    ...
    $event
      .children("header")
      .html(`<i class="fa fa-calendar-o"></i>` + calEvent.title);
    $event
      .find(".location")
      .text(
        calEvent.location ? calEvent.location : "(No location information.)"
      );
  ...
  if ($selectedElmt.hasClass("fc-highlight")) {
    leftPosition =
      $selectedElmt.offset().left - $popup.width() + $selectedElmt.width() / 2;
    if (leftPosition <= 0) {
      leftPosition = 5;
    ...
    prongPos =
      jsEvent.originalEvent.pageX - leftPosition - $prong.width() * 1.7;
  ...
  var topPosition =
    (calEvent ? jsEvent.originalEvent.pageY : $selectedElmt.offset().top) -
    $popup.height() -
    15;
  ...
  if (
    ...
    $prong
      .css("top", -($popup.height() + 12))
      .children("div:first-child")
      .removeClass("bottom-prong-dk")
      .addClass("top-prong-dk")
      .next()
      .removeClass("bottom-prong-lt")
      .addClass("top-prong-lt");
  } else {
    $popup.css("top", topPosition);
    $prong
      .css({ top: 0, bottom: 0 })
      .children("div:first-child")
      .removeClass("top-prong-dk")
      .addClass("bottom-prong-dk")
      .next()
      .removeClass("top-prong-lt")
      .addClass("bottom-prong-lt");
  }
...
$(document).ready(function() {
  $("#calendar").fullCalendar({
    ...
    eventRender: function(event, element) {
      element.append(
        `<span class='I_delete'><i class="fa fa-remove fa-2x"></i></span>`
      );
      element.append(
        `<span class='I_edit'><i class="fa fa-edit fa-2x"></i></span>`
      );
  ...
  $("#calendar-popup")
    ...
    .on("focusout", function(e) {
      ...
      if (
        $this.is(":visible") &&
        !$(e.relatedTarget).is("#calendar-popup, #calendar-popup *")
      ) {
        $this.hide();
      }
    });
  ...
  events.push(
    {
      id: 1,
      title: "event1",
      start: year + "-" + month + "-" + today + "T07:00",
      end: year + "-" + month + "-" + today + "T10:00",
      location: "The Moon",
      details: "There will be cheese"
    },
    {
      id: 2,
      title: "event2",
      start: year + "-" + month + "-" + tomorrow + "T03:00",
      end: year + "-" + month + "-" + tomorrow + "T08:00",
      location: "The Moon",
      details: "There will be cheese"
    },
    {
      id: 3,
      title: "event3",
      start: year + "-" + month + "-" + yesterday + "T20:00",
      end: year + "-" + month + "-" + today + "T05:00",
      location: "The Moon",
      details: "There will be cheese"
    }
  );
...
function initDialog() {
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    $(".dialog").append(
      '<div><span class="s_title">' +
        events[i].title +
        `</span><br><span class='s_des'>"` +
        events[i].details +
   ...
   <ul class="vertical-date">
            <li class="list-daynumber">` +
        events[i].start.slice(8, 10) +
        `</li>
            <li class="list-monthname">` +
        mS1[events[i].start.slice(5, 7)] +
        `</li>
          </ul>
